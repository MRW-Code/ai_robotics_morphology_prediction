...
class LabRepresentationGenerator:
    ...
    def gen_ml_set(self):
        print(f'Generating {args.input} for Lab API molecules')
        clean_desc = self.get_clean_descriptors(self.smiles, self.id)
        labels_df = self.raw_df.loc[:, ['api', 'eye_morphology']]
        df = pd.merge(labels_df, clean_desc, left_on='api', right_index=True)
        df = df.drop('api', axis=1)
        df = df.rename(columns={'eye_morphology' : 'label'})
        if self.save_output: df.to_csv(f'./lab_data/checkpoints/inputs/{args.input}_dataset.csv')
        return df
